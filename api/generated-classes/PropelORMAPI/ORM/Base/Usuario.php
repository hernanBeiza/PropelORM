...
abstract class Usuario implements ActiveRecordInterface
{
    ...
    /**
     * @var ObjectCombinationCollection Cross CombinationCollection to store aggregation of ChildTarea combinations.
     */
    protected $combinationCollTareaIdusuariotareas;

    /**
     * @var bool
     */
    protected $combinationCollTareaIdusuariotareasPartial;

    /**
     * @var        ObjectCollection|ChildTarea[] Cross Collection to store aggregation of ChildTarea objects.
     */
     ...
     * @var ObjectCombinationCollection Cross CombinationCollection to store aggregation of ChildTarea combinations.
     */
    protected $combinationCollTareaIdusuariotareasScheduledForDeletion = null;
    ...
    public function setValid($v)
    {
        ...
        if ($this->valid !== $v) {
            $this->valid = $v;
            $this->modifiedColumns[UsuarioTableMap::COL_VALID] = true;
        }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->collTareaIdusuariotareas = null;
        } // if (deep)
    }
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->combinationCollTareaIdusuariotareasScheduledForDeletion !== null) {
                if (!$this->combinationCollTareaIdusuariotareasScheduledForDeletion->isEmpty()) {
                    $pks = array();
                    foreach ($this->combinationCollTareaIdusuariotareasScheduledForDeletion as $combination) {
                        $entryPk = [];

                        $entryPk[1] = $this->getIdusuario();
                        $entryPk[2] = $combination[0]->getIdtarea();
                    ...
                    $this->combinationCollTareaIdusuariotareasScheduledForDeletion = null;
                }

            }

            if (null !== $this->combinationCollTareaIdusuariotareas) {
                foreach ($this->combinationCollTareaIdusuariotareas as $combination) {

                    //$combination[0] = Tarea (fkusuariotareaidtarea)
    ...
    public function setUsuariotareas(Collection $usuariotareas, ConnectionInterface $con = null)
    {
        ...
        foreach ($usuariotareasToDelete as $usuariotareaRemoved) {
            $usuariotareaRemoved->setUsuario(null);
        }
     ...
     * @return ObjectCollection|ChildUsuariotarea[] List of ChildUsuariotarea objects
     */
    public function getUsuariotareasJoinTarea(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildUsuariotareaQuery::create(null, $criteria);
        $query->joinWith('Tarea', $joinBehavior);

        return $this->getUsuariotareas($query, $con);
    }

    /**
     * Clears out the collTareaIdusuariotareas collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return void
     * @see        addTareaIdusuariotareas()
     */
    public function clearTareaIdusuariotareas()
    {
        $this->collTareaIdusuariotareas = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Initializes the combinationCollTareaIdusuariotareas crossRef collection.
     *
     * By default this just sets the combinationCollTareaIdusuariotareas collection to an empty collection (like clearTareaIdusuariotareas());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @return void
     */
    public function initTareaIdusuariotareas()
    {
        $this->combinationCollTareaIdusuariotareas = new ObjectCombinationCollection;
        $this->combinationCollTareaIdusuariotareasPartial = true;
    }

    /**
     * Checks if the combinationCollTareaIdusuariotareas collection is loaded.
     *
     * @return bool
     */
    public function isTareaIdusuariotareasLoaded()
    {
        return null !== $this->combinationCollTareaIdusuariotareas;
    }
     ...
     */
    public function createTareasQuery($idusuariotarea = null, Criteria $criteria = null)
    {
        $criteria = ChildTareaQuery::create($criteria)
            ->filterByUsuario($this);
    ...
    /**
     * Gets a combined collection of ChildTarea objects related by a many-to-many relationship
     ...
     * @param      ConnectionInterface $con Optional connection object
     *
     * @return ObjectCombinationCollection Combination list of ChildTarea objects
     */
    public function getTareaIdusuariotareas($criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->combinationCollTareaIdusuariotareasPartial && !$this->isNew();
        if (null === $this->combinationCollTareaIdusuariotareas || null !== $criteria || $partial) {
            if ($this->isNew()) {
                // return empty collection
                if (null === $this->combinationCollTareaIdusuariotareas) {
                    $this->initTareaIdusuariotareas();
                }
            } else {
                ...
                $combinationCollTareaIdusuariotareas = new ObjectCombinationCollection();
                foreach ($items as $item) {
                    $combination = [];

                    $combination[] = $item->getTarea();
                    $combination[] = $item->getIdusuariotarea();
                    $combinationCollTareaIdusuariotareas[] = $combination;
                }

                if (null !== $criteria) {
                    return $combinationCollTareaIdusuariotareas;
                }

                if ($partial && $this->combinationCollTareaIdusuariotareas) {
                    //make sure that already added objects gets added to the list of the database.
                    foreach ($this->combinationCollTareaIdusuariotareas as $obj) {
                        if (!call_user_func_array([$combinationCollTareaIdusuariotareas, 'contains'], $obj)) {
                            $combinationCollTareaIdusuariotareas[] = $obj;
                        }
                    }
                }

                $this->combinationCollTareaIdusuariotareas = $combinationCollTareaIdusuariotareas;
                $this->combinationCollTareaIdusuariotareasPartial = false;
            }
        }

        return $this->combinationCollTareaIdusuariotareas;
    }

    /**
     * Returns a not cached ObjectCollection of ChildTarea objects. This will hit always the databases.
     * If you have attached new ChildTarea object to this object you need to call `save` first to get
     * the correct return value. Use getTareaIdusuariotareas() to get the current internal state.
     *
     * @param int $idusuariotarea
     * @param Criteria $criteria
     * @param ConnectionInterface $con
     *
     * @return ChildTarea[]|ObjectCollection
     */
    public function getTareas($idusuariotarea = null, Criteria $criteria = null, ConnectionInterface $con = null)
    {
        return $this->createTareasQuery($idusuariotarea, $criteria)->find($con);
    }

    /**
     * Sets a collection of ChildTarea objects related by a many-to-many relationship
     * to the current object by way of the usuariotarea cross-reference table.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param  Collection $tareaIdusuariotareas A Propel collection.
     * @param  ConnectionInterface $con Optional connection object
     * @return $this|ChildUsuario The current object (for fluent API support)
     */
    public function setTareaIdusuariotareas(Collection $tareaIdusuariotareas, ConnectionInterface $con = null)
    {
        $this->clearTareaIdusuariotareas();
        $currentTareaIdusuariotareas = $this->getTareaIdusuariotareas();

        $combinationCollTareaIdusuariotareasScheduledForDeletion = $currentTareaIdusuariotareas->diff($tareaIdusuariotareas);

        foreach ($combinationCollTareaIdusuariotareasScheduledForDeletion as $toDelete) {
            call_user_func_array([$this, 'removeTareaIdusuariotarea'], $toDelete);
        }

        foreach ($tareaIdusuariotareas as $tareaIdusuariotarea) {
            if (!call_user_func_array([$currentTareaIdusuariotareas, 'contains'], $tareaIdusuariotarea)) {
                call_user_func_array([$this, 'doAddTareaIdusuariotarea'], $tareaIdusuariotarea);
            }
        }

        $this->combinationCollTareaIdusuariotareasPartial = false;
        $this->combinationCollTareaIdusuariotareas = $tareaIdusuariotareas;

        return $this;
    }

    /**
     * Gets the number of ChildTarea objects related by a many-to-many relationship
     * to the current object by way of the usuariotarea cross-reference table.
     *
     * @param      Criteria $criteria Optional query object to filter the query
     ...
     * @param      ConnectionInterface $con Optional connection object
     *
     * @return int the number of related ChildTarea objects
     */
    public function countTareaIdusuariotareas(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->combinationCollTareaIdusuariotareasPartial && !$this->isNew();
        if (null === $this->combinationCollTareaIdusuariotareas || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->combinationCollTareaIdusuariotareas) {
                return 0;
            } else {

                if ($partial && !$criteria) {
                    return count($this->getTareaIdusuariotareas());
                }
            ...
            return count($this->combinationCollTareaIdusuariotareas);
        }
    }

    /**
     * Returns the not cached count of ChildTarea objects. This will hit always the databases.
     * If you have attached new ChildTarea object to this object you need to call `save` first to get
     * the correct return value. Use getTareaIdusuariotareas() to get the current internal state.
     *
     * @param int $idusuariotarea
     ...
     * @return integer
     */
    public function countTareas($idusuariotarea = null, Criteria $criteria = null, ConnectionInterface $con = null)
    {
        return $this->createTareasQuery($idusuariotarea, $criteria)->count($con);
    }

    /**
     * Associate a ChildTarea to this object
     * through the usuariotarea cross reference table.
     *
     * @param ChildTarea $tarea,
     * @param int $idusuariotarea
     * @return ChildUsuario The current object (for fluent API support)
     */
    public function addTarea(ChildTarea $tarea, $idusuariotarea)
    {
        if ($this->combinationCollTareaIdusuariotareas === null) {
            $this->initTareaIdusuariotareas();
        }

        if (!$this->getTareaIdusuariotareas()->contains($tarea, $idusuariotarea)) {
            // only add it if the **same** object is not already associated
            $this->combinationCollTareaIdusuariotareas->push($tarea, $idusuariotarea);
            $this->doAddTareaIdusuariotarea($tarea, $idusuariotarea);
        }
     ...
     * @param int $idusuariotarea
     */
    protected function doAddTareaIdusuariotarea(ChildTarea $tarea, $idusuariotarea)
    {
        ...
        if ($tarea->isUsuarioIdusuariotareasLoaded()) {
            $tarea->initUsuarioIdusuariotareas();
            $tarea->getUsuarioIdusuariotareas()->push($this, $idusuariotarea);
        } elseif (!$tarea->getUsuarioIdusuariotareas()->contains($this, $idusuariotarea)) {
            $tarea->getUsuarioIdusuariotareas()->push($this, $idusuariotarea);
        }
    ...
    /**
     * Remove tarea, idusuariotarea of this object
     * through the usuariotarea cross reference table.
     *
     * @param ChildTarea $tarea,
     * @param int $idusuariotarea
     * @return ChildUsuario The current object (for fluent API support)
     */
    public function removeTareaIdusuariotarea(ChildTarea $tarea, $idusuariotarea)
    {
        if ($this->getTareaIdusuariotareas()->contains($tarea, $idusuariotarea)) {
            $usuariotarea = new ChildUsuariotarea();
            $usuariotarea->setTarea($tarea);
            if ($tarea->isUsuarioIdusuariotareasLoaded()) {
                ...
                $tarea->getUsuarioIdusuariotareas()->removeObject($this, $idusuariotarea);
            }
            ...
            $this->combinationCollTareaIdusuariotareas->remove($this->combinationCollTareaIdusuariotareas->search($tarea, $idusuariotarea));

            if (null === $this->combinationCollTareaIdusuariotareasScheduledForDeletion) {
                $this->combinationCollTareaIdusuariotareasScheduledForDeletion = clone $this->combinationCollTareaIdusuariotareas;
                $this->combinationCollTareaIdusuariotareasScheduledForDeletion->clear();
            }

            $this->combinationCollTareaIdusuariotareasScheduledForDeletion->push($tarea, $idusuariotarea);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep) {
            ...
            if ($this->combinationCollTareaIdusuariotareas) {
                foreach ($this->combinationCollTareaIdusuariotareas as $o) {
                    $o->clearAllReferences($deep);
                }
            }
        } // if ($deep)
        ...
        $this->collUsuariotareas = null;
        $this->combinationCollTareaIdusuariotareas = null;
    }
