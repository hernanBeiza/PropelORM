  ...
  public function __construct($c)
  {
    ...
    $this->logger->info(__CLASS__.":".__FUNCTION__."();");
		...
		if($usuario!=null){
			...
			$tareaCreada = TareaQuery::create()->findPK($tarea->getIdTarea());
	    $this->logger->info($tareaCreada);
			$respuesta["tarea"] = TareaVO::withTarea($tareaCreada)->jsonSerialize();

		} else {
			$respuesta = array("result"=>false,"tarea"=>null,"errores"=>"La tareas no se han guardado");
		}    	
    return $respuesta;
	}
    ...
    $this->logger->info(__CLASS__.":".__FUNCTION__."();");
		...
		if($tarea!=null){
			$tarea->setTitulo($tareaVO->getTitulo());
			$tarea->setValid($tareaVO->getValid());
			$tarea->save();
